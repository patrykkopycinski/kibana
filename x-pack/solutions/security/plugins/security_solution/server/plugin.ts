...
import { registerWorkflowSteps } from './lib/detection_engine/rule_types/vulnerability_check/workflow_steps';
    ...
    core
      ...
      .then(async ([coreStart, depsStart]) => {
        ...
        if (config.experimentalFeatures?.vulnerabilityCheckerEnabled) {
          const { installVulnerabilityCheckILMPolicies } = await import(
            './lib/detection_engine/rule_types/vulnerability_check/ilm_policies'
          );
          installVulnerabilityCheckILMPolicies(
            coreStart.elasticsearch.client.asInternalUser,
            this.logger
          ).catch((err) => {
            this.logger.warn(
              `[vulnerability_check] ILM policy installation failed (non-fatal): ${(err as Error).message
              }`
            );
          });
        }
