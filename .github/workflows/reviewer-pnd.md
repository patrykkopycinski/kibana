    ...
    (
      ...
      (
        (
          ...
          github.event.label.name == 'reviewer:pnd'
        ) ||
        (
          github.event.action != 'labeled' &&
          contains(github.event.pull_request.labels.*.name, 'reviewer:pnd')
        )
      )
    )
  )
...
- This reviewer's own gh-aw workflow id is `reviewer-pnd`. Use it as "this reviewer's own workflow id" when matching review threads to resolve.
