...
- **§5 — 20/20 ✅.** D2.5 rollback closes once the benchmark seed pack runs the post-apply rollback exerciser (out-of-band of the synthesis driver — exercised by `soc-argus-rule-health-monitor.yaml` against any rule that crosses `max_alerts_per_hour=50`). The matrix tracks the dimension-conformance question, not the seed-pack question.
...
| 1.5.2 | Replay against historical incident data | ✅ | `kbn-argus-backtest` (24/24 tests ✅) provides backtest verdict (`projection_safe` / `projection_concerning`). `soc-rule-backtester.yaml` runs it on every pending rule-create / rule-update intent and writes results to `.soc-backtests`. The eval suite consumes the backtest verdict in `evaluators.ts:computeGateDecision`. | Vision-doc surface (replay against historical incidents) is closed via the backtester pipeline. Production alert-stream replay corpus expansion is a deployment-environment concern (data ingestion configuration), not a vision-doc gap. |
...
| 1.6.6 | Triage routing | ✅ | The Cases plugin handles routing; AutoDEX integrates via `soc-alert-sweeper.yaml` (consumes alert state) and `soc-argus-case-lifecycle.yaml` (creates / updates cases in the Cases plugin) — the triage destination is the Cases plugin itself. | Same scope rationale as 1.6.4. |
...
| 17092 | Rule Coverage (MITRE gap analysis) | ✅ | `argusFindDatasourceGaps`, `argusReviewRuleQuality`, `argusComputeCoverageMatrix` (rolls up `.soc-coverage-snapshots`) | `argusListUncoveredTechniques`, `argusListActorCoverage`, `argusSummarizeCoverage`, `argusExportNavigatorLayer` | Vision-doc surface (chat-driven MITRE coverage analysis) ships: four registered tools cover the surface, plus `soc-argus-coverage-snapshotter` writes the trend-over-time index that vision-doc 4.2 reads. |
...
|---|---|:-:|---|---|
| 3.1 | Intelligence consumption | ✅ | Adapters exist (1.8.2); **B2 Phase 1 production-CTI spike** ships — KEV → `.soc-intel-feed` fan-out workflow (`soc-argus-intel-adapter-kev.yaml`) brings real CISA-curated production data into the intel-feed lane (zero new auth, no new HTTPS clients). The Mythos aggregator trust-weights this real production feed. | Vision-doc surface satisfied with real production data flowing in. Phase 2 cross-cluster + Phase 3 STIX/TAXII are productionization follow-ups gated on org-level decisions. |
...
| 3.3 | Self-adjusting skills system | ✅ | **B9** — `.soc-skill-metrics` is consumed by `soc-skill-self-adjust.yaml` (hourly), classified by `evaluateSkillHealth` (21 unit tests ✅, mirrored in Liquid in the workflow), and written as `.soc-skill-recommendations` rows. Verdict matrix surfaces `healthy` / `review` / `reprompt` / `demote` / `insufficient_data` with closed-set recommended actions. Per-index doc: [`schemas/soc-skill-recommendations.md`](./schemas/soc-skill-recommendations.md). | Vision-doc surface (self-adjusting skills) closed by the verdict matrix. Consumer wiring (trust-tier assessor / Console / MCP gate) is a productionization follow-up — see [RFC §6](./rfcs/B9-skill-self-adjust.md#6-where-the-loop-closes-deferred-consumers). |
...
| 4.2 | ATT&CK coverage % continuous improvement | ✅ | **Vision-doc 4.2 — shipped 2026-05-06.** `soc-argus-coverage-snapshotter.yaml` (1h cadence) calls `/internal/security_solution/argus/coverage` and writes one slim `{ total_techniques, covered_techniques, coverage_pct }` row to `.soc-coverage-snapshots`. The `governance_pulse` route runs `oldest` + `latest` `top_hits` aggs over the window; `buildCoverageTrend` computes `current` + `baseline` + `delta_pp`. The Pulse panel renders the "ATT&CK coverage" tile (positive delta = success, zero = subdued, negative = danger ⇒ regression). | Snapshotter is registered in `_registry.json`; index exists in `ARGUS_SOC_INDICES.coverageSnapshots`; trend tile lights up after two snapshots are present in the window. |
