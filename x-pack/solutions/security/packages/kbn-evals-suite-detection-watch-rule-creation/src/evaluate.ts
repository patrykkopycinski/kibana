...
>({
  ...
  ruleCreationClient: [
    async ({ fetch, log, traceEsClient }, use) => {
      // traceEsClient drives the routing evaluator. When the stack booted without a trace ES it is
      // undefined and the routing evaluator reports N/A rather than a false zero.
      const client = new RuleCreationClient(fetch, log, traceEsClient);
      await use(client);
