...
/**
 * Asserts the managed rule-creation workflow the security_solution plugin installs at start is
 * present, and returns its yaml.
 *
  ...
  try {
    ...
    throw new Error(
      `Managed workflow "${RULE_CREATION_WORKFLOW_ID}" is not installed. It is installed at ` +
        `plugin start by security_solution (server/workflows/rule_creation_workflow/install.ts), ` +
        `so this usually means the Workflows feature is disabled or the stack booted without the ` +
        `security solution plugin. This suite does not create the workflow itself: it must ` +
        `measure the workflow that ships. ` +
        `Original error: ${err instanceof Error ? err.message : String(err)}`
    );
  }
};
