...
import type { Config, Lifecycle, DockerServersService, EsVersion, DedicatedTaskRunner } from './lib';
...
type ProvidedTypeMap<T extends {}> = {
    [K in keyof T]: T[K] extends new (...args: any[]) => infer X ? X : T[K] extends (...args: any[]) => any ? ProvidedType<T[K]> : unknown;
};
export interface GenericFtrProviderContext<ServiceProviders extends {}, PageObjectProviders extends {}, ServiceMap = ProvidedTypeMap<ServiceProviders>, PageObjectMap = ProvidedTypeMap<PageObjectProviders>> {
    /**
     * Determine if a service is avaliable
     * @param serviceName
     */
    hasService(serviceName: 'config' | 'log' | 'lifecycle' | 'dockerServers' | 'esVersion' | 'dedicatedTaskRunner'): true;
...
export declare class GenericFtrService<ProviderContext extends GenericFtrProviderContext<any, any>> {
