...
import { type DatasetMaturity, type EvaluationScoreDocument, type IngestScoresRequestBodyInput, type Model as EvalsModel } from '@kbn/evals-common';
...
export declare class EvalsClient {
    ...
    constructor(kbnClient: KbnClient, log: SomeDevLog, { spaceIds }?: {
        spaceIds?: string[];
    });
    private path;
    ingestScores(request: IngestScoresRequestBodyInput): Promise<IngestScoresResult>;
    getExperimentStats(experimentId: string, options?: GetExperimentFilters): Promise<ExperimentStats | null>;
    getExperimentScores(experimentId: string, options?: GetExperimentFilters): Promise<EvaluationScoreDocument[]>;
    /**
    ...
    findLatestExperimentForBuild({ suiteId, branch, baseExecutionId, }: {
        suiteId: string;
        branch?: string;
        baseExecutionId: string;
    }): Promise<BaselineExperiment | undefined>;
    findLatestBaselineExperiment({ suiteId, branch, taskModelId, excludeExecutionId, }: {
        suiteId: string;
        branch: string;
        taskModelId?: string;
