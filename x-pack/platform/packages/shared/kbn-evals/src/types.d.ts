...
import type { EvaluatorDisplayOptions, EvaluatorDisplayGroup } from './utils/reporting/report_table';
...
export interface EvaluationDatasetWithId<TExample extends Example = Example> extends Omit<EvaluationDataset<TExample>, 'id'> {
    id: string;
}
export type TaskOutput = unknown;
export interface Example<TInput extends Record<string, unknown> = Record<string, unknown>, TExpected = any, TMetadata extends Record<string, unknown> | null = Record<string, unknown> | null> {
    /**
...
type EvaluatorCallback<TExample extends Example, TTaskOutput extends TaskOutput> = (params: EvaluatorParams<TExample, TTaskOutput>) => Promise<EvaluationResult>;
/**
...
export interface Evaluator<TExample extends Example = Example, TTaskOutput extends TaskOutput = TaskOutput> {
    name: string;
    kind: EvaluatorKind;
...
export type ExperimentTask<TExample extends Example, TTaskOutput extends TaskOutput> = (example: TExample) => Promise<TTaskOutput>;
/**
...
export interface EvalsExecutorClient {
    runExperiment<TEvaluationDataset extends EvaluationDataset, TTaskOutput extends TaskOutput = TaskOutput>(options: {
        /**
