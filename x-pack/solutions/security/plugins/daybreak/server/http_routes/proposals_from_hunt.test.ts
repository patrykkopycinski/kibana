...
describe('proposals_from_hunt route', () => {
  it('registers POST /api/daybreak/proposals/from-hunt', () => {
    const deps = createMockDependencies();
    registerProposalsFromHuntRoute(deps);
    expect(deps.router.post).toHaveBeenCalledWith(
      expect.objectContaining({ path: '/api/daybreak/proposals/from-hunt' }),
      expect.any(Function)
    );
  });
});
