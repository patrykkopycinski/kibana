...
//   - SECURITY_RULE_CREATION_WORKFLOW_ID (@kbn/workflows/managed)
...
/**
 * Managed workflow id, installed globally by the security_solution plugin at start
 * (see server/workflows/rule_creation_workflow/install.ts). The eval drives this exact document —
 * it does not create or carry its own copy, so eval and production cannot drift.
 */
