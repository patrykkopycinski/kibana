# AutoSOC structured-output schemas

These JSON Schemas (draft-2020-12) are the **contract** between AutoSOC agents
...
- LLM free-text output drifts every model release. Parsing it with regex has
  caused roughly 70 % of AutoSOC's apply-time failures.
...
1. Every AutoSOC agent is given the relevant schema inline in its `instructions`
...
| Schema | Purpose |
|---|---|
| `recommendation.schema.json` | Envelope schema — every AutoSOC recommendation must validate against this. Uses a `type`-discriminated union. |
