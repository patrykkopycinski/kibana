...
export const renderMatrixHtml = (
  ...
  const modelCards =
    (matrix.proprietary.length > 0 ? renderModelCard(matrix.proprietary, matrix, traces) : '') +
    (matrix.openSource.length > 0 ? renderModelCard(matrix.openSource, matrix, traces) : '');
