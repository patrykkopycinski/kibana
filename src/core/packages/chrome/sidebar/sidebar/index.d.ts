...
export type { SetState, GetState, SidebarContext, SidebarStoreConfig, } from './src/create_sidebar_store';
export { createSidebarStore } from './src/create_sidebar_store';
/** Production sidebar app IDs */
export declare const VALID_SIDEBAR_APP_IDS: readonly ['agentBuilder'];
/** Prefix for example/test app IDs */
export declare const EXAMPLE_APP_ID_PREFIX = "sidebarExample";
...
export declare function isValidSidebarAppId(appId: string): appId is SidebarAppId;
/**
 * Props passed to sidebar app components.
 * - With store: receives state + actions + onClose
 * - Without store: receives only onClose
 */
export type SidebarComponentProps<TState = undefined, TActions = undefined> = TState extends undefined ? {
    onClose: () => void;
} : {
    state: TState;
    actions: TActions;
...
export type SidebarComponentType<TState = undefined, TActions = undefined> = ComponentType<SidebarComponentProps<TState, TActions>>;
...
export type SidebarAppDefinition<TState = undefined, TActions = undefined> = SidebarAppConfig<TState, TActions> & {
    status: SidebarAppStatus;
    restoreOnReload: boolean;
};
...
export interface SidebarStart {
    ...
    /** Get app-bound API */
    getApp: <TState = undefined, TActions = undefined>(appId: SidebarAppId) => SidebarApp<TState, TActions>;
    /** Get app definition */
